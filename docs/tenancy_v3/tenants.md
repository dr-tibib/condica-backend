...
```php
$tenant->update([ 'attributeThatHasNoColumn' => 'value', // stored in the `data` JSON column 'plan' => 'free' // stored in the dedicated `plan` column (see below) ]);
```

or

```php
$tenant->customAttribute = 'value'; // stored in the `data` JSON column $tenant->plan = 'free'; // stored in the `plan` column (see below) $tenant->save();
```
...
```php
public static function getCustomColumns(): array { return [ 'id', 'plan', ]; }
```
...
```php
public static function getDataColumn(): string { return 'my-data-column'; }
```
