...
The package expects the assets to be stored in your tenant's app/public/ directory. For global assets (non-private assets shared among all tenants), you may want to create a disk and use URLs from that disk instead.
...
You can disable tenancy of asset() in the config (tenancy.filesystem.asset_helper_tenancy) and explicitly use tenant_asset() instead. tenant_asset() always returns a path to the TenantAssetController.
