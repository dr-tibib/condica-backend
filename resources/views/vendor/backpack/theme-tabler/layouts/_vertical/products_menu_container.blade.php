@if (backpack_auth()->check())
    <aside data-menu-theme="{{ $theme ?? 'system' }}" class="{{ backpack_theme_config('classes.sidebar') ?? 'navbar navbar-vertical navbar-expand-lg navbar-'.($theme ?? 'light') }} @if(backpack_theme_config('options.sidebarFixed')) navbar-fixed @endif">
        <div class="container-fluid">
            ...
            <h1 class="navbar-brand d-none d-lg-block align-self-center mb-3">
                <span class="h2 text-decoration-none mb-0">Products</span>
            </h1>
            ...
            <div class="collapse navbar-collapse" id="mobile-menu">
                <ul class="navbar-nav pt-lg-3">
                    ...
                    @include(backpack_view('layouts.partials.products_sidebar_content'))
                </ul>
            </div>
        </div>
    </aside>
