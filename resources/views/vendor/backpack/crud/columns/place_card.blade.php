...
<div class="d-flex align-items-center p-2 border rounded bg-white" style="max-width: 350px;">
    @if($imageUrl)
        <img src="{{ $imageUrl }}" alt="{{ $name }}" class="rounded mr-3" style="width: 80px; height: 80px; object-fit: cover;">
    @else
        <div class="rounded mr-3 d-flex align-items-center justify-content-center bg-light text-secondary" style="width: 80px; height: 80px;">
            <i class="la la-map-marker la-2x"></i>
        </div>
    @endif

    <div style="overflow: hidden;">
        <div class="font-weight-bold text-truncate" title="{{ $name }}">{{ $name }}</div>
